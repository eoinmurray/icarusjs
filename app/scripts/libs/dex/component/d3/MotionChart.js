...
MotionChart.constructor = MotionChart;
...
function MotionChart(userConfig)
{
  DexComponent.call(this, userConfig,
  {
    ...
    'csv'              :
    {
    	...
    	'header'         : [ "time", "x", "y", "name", "category", "size" ],
    	'data'           : [
    	                     [2010, 1, 1, "NAME1", "C1", 1],
    	                     [2010, 2, 2, "NAME2", "C2", 2],
    	                     [2011, 3, 3, "NAME1", "C1", 2],
    	                     [2011, 4, 4, "NAME2", "C2", 1],
    	                     [2012, 1, 1, "NAME1", "C1", 1],
    	                     [2012, 25, 25, "NAME2", "C2", 20],
    	                     [2013, 50, 50, "NAME1", "C1", 2],
    	                     [2013, 100, 100, "NAME2", "C2", 1]
    	                   ]
    },
    'index'      :
    {
...
MotionChart.prototype.render = function()
{
  this.update();
};

MotionChart.prototype.update = function()
{
...
*/
  ...
  function x(d) { return d[config.index.x]; }
  function y(d) { return d[config.index.y]; }
  function radius(d) { return d[config.index.size]; }
  function color(d) { return d[config.index.category]; }
  function key(d) { return d[config.index.name]; }
  ...
  var timeFontSize = 18;
  var timeLabelSvg = config.timeLabel;
  ...
  var dot = chartContainer.append("g")
    ...
    .enter().append("circle")
      ...
      .style("fill", function (d) { return config.color(d[0][config.index.category]); })
      .call(position)
      .sort(order)
...
//.on("mouseout", function(){d3.select(this).style("fill", "white");})
  ...
  {
    dot .attr("cx", function(d) { return config.xaxis.scale(x(d[0])); })
        .attr("cy", function(d) { return config.xaxis.scale(y(d[0])); })
        .attr("r", function(d) { return radiusScale(radius(d[0])); });
  }
  ...
  function interpolateData(time)
  {
    ...
    return timeGroup.data.map(function(od) { return od.csv.data.map(function(d)
    {
